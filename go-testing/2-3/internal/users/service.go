...
type Service interface {
	...
	CreateUser(string, string, string, int, float64, bool, string) (User, error)
...
func (s *service) CreateUser(name string, lastName string, email string, age int, height float64, active bool, creation string) (User, error) {
	newId, err := s.repository.LastId()
	if err != nil {
		return User{}, err
	}	
	us, err := s.repository.CreateUser(newId, name, lastName, email, age, height, active, creation)
	if err != nil {
		return User{}, err
	}
